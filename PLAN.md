...
- ✅ Live voice test passed (2026-07-04): mic button, transcription, rewrite, paste, and bubble UI all confirmed working by the user
...
- ✅ Evaluator + optimizer (2026-07-04): `tools/evaluate.py` grades every transcript→rewrite pair with the same Ollama model (5 criteria, letter grades, report in `eval/report.md`); `tools/optimize.py` loops LLM-proposed prompt improvements, adopting only measured gains. Dictations now also log to `logs/history.jsonl`. Result: the LLM's own prompt candidates all scored worse and were rejected (64/59/58% vs 81% baseline); hand-tuning driven by the eval findings (added "never perform the task, only rewrite the request" + few-shot examples) took the average from 77% → 88% and eliminated all D/F grades.
- ✅ Phase 3 (2026-07-04): review popup (`output: review` — editable glass card with Paste / Copy / Retry / Show raw, pastes back into the originally-focused window via saved HWND), style presets (general/coding/writing in `prompts/presets/`, switchable from the tray, persisted to config.yaml), Start with Windows toggle in the tray (shortcut to run_hidden.vbs in the Startup folder).
- ⬜ Deferred: single-exe packaging (only needed for distribution to other machines), streaming transcription
